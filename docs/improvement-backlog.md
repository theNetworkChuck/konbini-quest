...
### 2026-08-12 -- #44 Payday / Month-End Ambient NPC Lines
**What was added:**
- Fourth context axis for ambient street chatter, layered after time-of-day (35%), weather (30%), and weekday-vs-weekend (25%). Every ambient bubble roll now has a 20% chance of pulling from a payday-week / regular / broke pool based on the calendar day-of-month, before falling through to the NPC's personality pool. Bubbles from all four axes now coexist so the street reflects the hour AND the weather AND weekday-vs-weekend AND where salarymen sit in the monthly payday cycle.
- **3 day-of-month buckets** (based on JS `Date().getDate()`):
  - **payday_week (25 - end of month)** 8 lines: 給料日だ！/ 今日は少し贅沢しよう / ボーナス入った / ここのプリンも買おうかな / 今週はちょっといいビールにしよう / お金あると心に余裕がでる / 今月よく頑張った / デザート二つもらっちゃおう
  - **regular (5 - 24)** 6 lines: 普通にコーヒーだけ / ちょうど中間くらい / この値段ならいいか / 弁当にしとこう / まだ余裕ある / もうすぐ給料日
  - **broke (1 - 4, month-end wallet-recovery days)** 8 lines: 給料日まであと少し / 今月ピンチ / お金ない… / 一番安いのでいいや / おにぎり一つだけ / 財布寒い… / PayPayで払おう / もう一週間の辛抱
- **Coach-type NPCs** (sensei / paymentcoach / seasonalguide / etc) are gated to half the payday chance (10% for coaches vs 20% for regular NPCs) so they stay on-topic about Japanese teaching more often.
- **New test hook:** `window.forcePaydayBucket('payday_week' | 'regular' | 'broke' | null)` overrides the real calendar reading; pass null to reset. Also exposed as `NPCs.setPaydayOverride()`.
- Empirical verification with 100 samples per bucket surfaced all 22 lines across the three pools with roughly uniform distribution -- 8/8 payday_week lines, 6/6 regular lines, 8/8 broke lines appeared. Bad bucket names are silently rejected (setter leaves previous override untouched).
- Boot check: 100% non-black canvas, no overlay, zero errors. Same defensive posture as previous ambient additions.

**Why it matters for learning:** In Japan the 25th of the month is the traditional salaryman payday (給料日 kyuuryoubi), and this monthly rhythm shapes an enormous amount of everyday spending behavior and vocabulary that textbooks completely ignore. On the 26th people buy the slightly nicer beer; on the 3rd people buy the cheapest onigiri and count their remaining yen. Neither JLPT nor typical classroom Japanese teaches this monthly-payday cycle vocabulary because it's an implicit cultural rhythm rather than a grammar point, but it's exactly the vocab a learner overhears at every register line in Osaka. The pool teaches (a) core money-culture words -- 給料日 (payday), ボーナス (bonus), 贅沢 (luxury/splurge), 余裕 (financial breathing room), ピンチ (in a bind), 財布寒い (wallet's cold, an idiom for broke), 辛抱 (endurance/patience -- the exact word salarymen use waiting for payday); (b) the two payment verbs learners often confuse in konbini context -- 買おう (I'll buy) and 払おう (I'll pay), including the very high-frequency real phrase PayPayで払おう which reflects Japan's rapid QR-payment adoption; and (c) the plain-form volitional pattern (しよう / 買おう / 払おう) that natives use constantly for talking-to-oneself thinking-out-loud speech and that textbooks introduce late because it looks messy grammatically. Because the pool's bucket flips with the actual calendar day, a learner playing on the 27th hears the payday-flush vocabulary automatically and a learner playing on the 2nd hears the wallet-recovery vocabulary -- forcing the language exposure to match the exact economic mood of the real-world day they're playing on. This is one of the most passive, natural-context ways to teach culturally-loaded vocabulary that never makes it into formal study.

**Files modified:** npc.js (+~85 lines: PAYDAY_LINES with 3 buckets × 6-8 lines = 22 authentic phrases, PAYDAY_BUBBLE_CHANCE = 0.20 constant, _paydayOverride module state, setPaydayOverride with allow-list guard, getPaydayBucket with day-of-month bucketing wrapped in try/catch, pickPaydayLine helper, fourth stage in updateAmbientBubbles inner picker after day-type before personality fallback, coach detection halves this chance too, exports). game.js (+7 lines: window.forcePaydayBucket(bucket) test hook).
